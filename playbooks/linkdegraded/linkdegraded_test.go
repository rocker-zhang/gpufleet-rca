...
func TestLinkDegraded_Match(t *testing.T) {
	...
	cases := []struct {
		...
		// same-device guard cases
		{
			...
			window:    []rca.Evidence{evd("link.error", dcgm, "GPU-7"), evd("link.degraded.pcie", prom, "GPU-7")},
			wantFired: true,
			wantIDs:   []string{"link.error", "link.degraded.pcie"},
		},
		{
			name:      "different device -> no fire",
			window:    []rca.Evidence{evd("link.error", dcgm, "GPU-0"), evd("link.degraded.pcie", prom, "GPU-9")},
			wantFired: false,
		},
		{
			name:      "one leg missing device -> FIRE (attribution unavailable)",
			window:    []rca.Evidence{evd("link.error", dcgm, "GPU-0"), evd("link.degraded.pcie", prom, "")},
			wantFired: true,
			wantIDs:   []string{"link.error", "link.degraded.pcie"},
		},
		{
			...
			window: []rca.Evidence{
				evd("link.error", dcgm, "GPU-0"),
				evd("link.degraded.cross", prom, "GPU-9"),
				evd("link.degraded.same", prom, "GPU-0"),
			},
			wantFired: true,
			wantIDs:   []string{"link.error", "link.degraded.same"},
		},
	}
